...
export class UtilityMethods {
    ...
    //curl -X GET --header 'Accept: application/json' 'http://localhost:8080/api/v1/diagram/100'
    public readDBDiagram(diagramID: string, selectedItem: SelectorViewModel): void {
        (document.getElementsByClassName('sb-content-overlay')[0] as HTMLDivElement).style.display = '';
        let url = 'http://localhost:8080/api/v1/diagram/' + diagramID;
        let ajax: Ajax = new Ajax(url, 'GET', true);
        ajax.send().then();
        let context: any = this;
        ajax.onSuccess = (data: string): void => {
            selectedItem.preventSelectionChange = true;
